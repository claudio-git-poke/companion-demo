/* ============================================================
   ...
   Tutto il disegno e' generato da codice: nessun file immagine.
   ============================================================ */
...
(function (global) {
  ...
  var DEFAULTS = {
    ...
    // Economia
    dustPerDailyPet: 5,       // polvere magica al primo coccolone del giorno
    xpPerPet: 1,              // punti affetto per coccolone
    maxXpPerDay: 12,          // tetto giornaliero di punti affetto
  ...
  function loadState() {
    ...
    state = saved && typeof saved === 'object' ? saved : blankState();
  ...
  CornerWidget.prototype.pet = function () {
    var result = petActive();
    this.sprite.setPose(result.excited ? 'excited' : 'happy', 900);
    this.root.classList.remove('is-squash');
    void this.root.offsetWidth;
    this.root.classList.add('is-squash');
    this.burst(result.leveledUp ? 'level' : 'heart');

    if (result.leveledUp) {
      this.say('Livello affetto ' + result.level + '!', 3200);
    } else if (result.dust > 0) {
      this.say('+' + result.dust + ' polvere magica', 3000);
    } else {
      this.say(pickRandom(result.creature.lines[result.excited ? 'excited' : 'happy']));
    }
  };
  ...
  function petActive() {
    ...
    if (state.xpToday < cfg.maxXpPerDay) {
      rec.xp += cfg.xpPerPet;
      state.xpToday += cfg.xpPerPet;
    }
    rec.pets += 1;
    state.lastPetAt = Date.now();

    if (state.lastDustDay !== day) {
      state.lastDustDay = day;
      dust = cfg.dustPerDailyPet;
    }

    var afterLevel = levelFor(rec.xp);
    if (afterLevel > beforeLevel) leveledUp = true;
    ...
    return {
      ...
      level: afterLevel,
      leveledUp: leveledUp,
      excited: leveledUp || rec.pets % 7 === 0
    };
  }
